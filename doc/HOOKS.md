...
(resp. receive events from) *Cadmus*. An event is a JSON object with a required
...
When a directory is deselected, *Cadmus* will send the `SIGTERM` signal to all
